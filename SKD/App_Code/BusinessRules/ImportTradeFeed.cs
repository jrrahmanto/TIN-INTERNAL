...
/// </summary>
public class ImportTradeFeed : TextImportHandler
{
    ...
    private TradefeedData.RawTradeFeedDataTable dt;
    ...
    public ImportTradeFeed(Type recordType, string userName, decimal exchID, DateTime bussinessDate)
        : base(recordType)
	{
        user = userName;
        exchangeID = exchID;
    ...
    public override void ProcessRow(object obj)
    {
        try
        {
            ...
            if (dt == null)
            {
                dt = new TradefeedData.RawTradeFeedDataTable();
            }

            dt.AddRawTradeFeedRow(exchangeID, _identifier, bussDate, _tradeTime, DateTime.Now,
                                  _offset, _optionCode, decimal.Parse(_settlementMonth), _price, _qty, _contraIndicator,
                                  _exchangeReference, _sellerClearingMemberCode, _sellerMemberGiveUpCode,
                                  _sellerGiveUpCommission, _sellerReference, _sellerTraderTypeCode,
                                  _sellerCompositeTradeTypeCode, _sellerTotalLegs, _buyerAccountCode,
                                  _buyerTraderTypeCode, _buyerMemberGiveUpCode, _buyerGiveUpCommission,
                                  _buyerCompositeTradeTypeCode, _buyerReference, _strikePrice, _tradeVersion,
                                  _buyerTotalLegs, _sellerAccountCode, _buyerClearingMemberCode,
                                  _productCode, _sellerExchangeMemberCode, _buyerExchangeMemberCode,
                                  user);
        }
        catch (Exception ex)
        {
    ...
    public override void Deinitialize()
    {
        ...
        else
        {
            try
            {
                Tradefeed.InsertRawTradeFeed(dt);
            }
            catch (Exception ex)
            {
                throw ex;
